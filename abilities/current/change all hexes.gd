...
func activate(Game,entity,val):
	for hex in get_tree().get_nodes_in_group('Hex'):
		if hex.id>0 and not hex.stateLocal.hex_type == hex.STATE_ORB and not hex.stateLocal.hex_type == hex.STATE_WELL:
			if (
				(affect_empty and hex.stateLocal.hex_type == hex.STATE_EMPTY)
				or not hex.stateLocal.hex_type == hex.STATE_EMPTY
			): 
