...
func activate(game, entity, target):
	var name = entity.Unit.set_mod_att(buff_name, power, true)
	var inst_unbuff = unbuffer.instance()
	inst_unbuff.init(entity)
	inst_unbuff.add_mod_name(name)
	entity.Unit.get_node('on_end_turn').add_child(inst_unbuff)
